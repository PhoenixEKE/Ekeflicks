...
class ProducerAnalyticsViewSet(viewsets.ViewSet):
    ...
    - ownership comes from Content.producer;
    - client-supplied producer identifiers do not define scope;
    ...
    def _producer_allowed(self, request):
        ...
        return is_active_producer_user(user)

    def list(self, request):
        if not self._producer_allowed(request):
            return Response(
                {
                    'detail': (
        ...
        content_ids = producer_analytics_content_ids(
            request.user
        )
