...
class ProducerAnalyticsViewSet(viewsets.ViewSet):
    ...
    def content_detail(
        ...
        """
        Producer-owned analytics detail for one content.

        Critical security order:
        1. authenticate / producer eligibility;
        2. reject client producer overrides;
        3. validate reporting window;
        4. resolve Content ownership in PostgreSQL;
        5. only then query ClickHouse for that single Content ID.

        Foreign and unknown contents both return 404.
        ClickHouse is never called for either case.
        """
        import uuid
        ...
        # Normalize malformed UUIDs to the same 404 contract
        # as unknown / foreign content, without touching CH.
        try:
            normalized_content_id = uuid.UUID(
                str(content_id)
            )
        except (
            TypeError,
        ...
        # Ownership is now established in PostgreSQL.
        # Only from this point is ClickHouse allowed.
        from apps.analytics.services import (
        ...
        if rows:
            ...
            # even though CH was constrained to one allowed UUID,
            # never expose a mismatching row.
            if str(
                row.get(
                    'value'
                )
