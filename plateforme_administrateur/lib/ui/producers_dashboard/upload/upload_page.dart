...
class _UploadPageState extends State<UploadPage> {
  ...
  static const List<String> allGenres = [
    'Action', 'Aventure', 'Comédie', 'Drame', 'Documentaire', 'Horreur', 
    'Romance', 'Science-Fiction', 'Fantastique', 'Thriller', 'Animation',
    'Musical', 'Biopic', 'Famille', 'Historique', 'Policier', 'Sport',
  ];
  ...
  Future<void> _pickGlobalPoster(bool isPoster) async {
    ...
    if (p == null) return;
    setState(() {
      if (isPoster) posterFile = File(p);
      else bannerFile = File(p);
    });
  }
  ...
  void _saveForm() {
    if (!_formKey.currentState!.validate()) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.formValidationError)),
      );
      return;
    }
    ...
    if (selectedGenres.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.genreSelectionError)),
      );
      return;
    }
    ...
    if (selectedLanguage == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.languageSelectionError)),
      );
      return;
    }
    if (selectedLanguage == LanguageOption.autres && languageOtherController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.languageSpecifyError)),
      );
      return;
    }
    ...
    if (selectedCountry == null || selectedCountry!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.countrySelectionError)),
      );
      return;
    }
    if (selectedCountry == "Autres" && countryOtherController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.countrySpecifyError)),
      );
      return;
    }
    ...
    if (releaseYear == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.yearSelectionError)),
      );
      return;
    }
    ...
    if (directorNameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.directorNameError)),
      );
      return;
    }
    if (screenwriterNameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.screenwriterNameError)),
      );
      return;
    }
    ...
    if (_contentType != ContentType.serie) {
      if (posterFile == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context)!.posterSelectionError)));
        return;
      }
      if (bannerFile == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context)!.bannerSelectionError)));
        return;
      }
      if (videoFile == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context)!.videoSelectionError)));
        return;
      }
    } else {
      if (seasons.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context)!.seasonSelectionError)),
        );
        return;
      }
      for (final w in seasons) {
        ...
        if (!valid) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(AppLocalizations.of(context)!.seasonValidationError)),
          );
          return;
        }
      }
    ...
    print("Titre : ${titleController.text.trim()}");
    print("Description : ${descriptionController.text.trim()}");
    print("Type : $_contentType");
    print("Genres : ${selectedGenres.toList()}");
    print("Langue : ${selectedLanguage == LanguageOption.autres ? languageOtherController.text.trim() : selectedLanguage}");
    print("Pays : ${selectedCountry == 'Autres' ? countryOtherController.text.trim() : selectedCountry}");
    print("Année : $releaseYear");
    print("Réalisateur : ${directorNameController.text.trim()}, image: $directorImagePath");
    print("Scénariste : ${screenwriterNameController.text.trim()}, image: $screenwriterImagePath");
    print("Nb producteurs: ${producers.length}, nb acteurs: ${actors.length}");
    print("Nb saisons: ${seasons.length}");

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(AppLocalizations.of(context)!.formSubmissionSuccess)),
    );
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(l10n.contentTypeLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            ...
            TextFormField(
              ...
              validator: (v) => v == null || v.isEmpty ? l10n.requiredFieldError : null,
            ),
            ...
            // GENRES
            Text(l10n.genresLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Wrap(
              spacing: 8,
            ...
            if (selectedLanguage == LanguageOption.autres) ...[
              ...
              TextFormField(
                ...
                validator: (v) {
                  if (selectedLanguage == LanguageOption.autres && (v == null || v.isEmpty)) {
            ...
            // PAYS
            Text(l10n.countryLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            DropdownButtonFormField<String>(
              value: selectedCountry,
              items: [
                ...africanCountries.map((c) => DropdownMenuItem(
                  value: c, 
                  child: Text(c))
                ),
                DropdownMenuItem(
            ...
            // Année
            Text(l10n.releaseYearLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            ...
            Text(l10n.productionTeamTitle, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
              ...
              Column(children: [
                ...
                if (directorImagePath != null)
                  SizedBox(
                    width: 64, 
                    height: 64, 
                    child: Image.file(File(directorImagePath!), fit: BoxFit.cover)
                  ),
              ]),
            ]),
            const SizedBox(height: 12),
            ...
            Row(children: [
              ...
              Column(children: [
                ...
                if (screenwriterImagePath != null) const SizedBox(height: 6),
                if (screenwriterImagePath != null)
                  SizedBox(
                    width: 64, 
                    height: 64, 
                    child: Image.file(File(screenwriterImagePath!), fit: BoxFit.cover)
                  ),
              ]),
            ]),
            const SizedBox(height: 20),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(l10n.producersLabel, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ElevatedButton(
            ...
            // Acteurs
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(l10n.actorsLabel, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ElevatedButton(
                onPressed: _addActor, 
                child: Text(l10n.addButton),
                style: ElevatedButton.styleFrom(
            ...
            // --- Saisons (si série) ---
            if (_contentType == ContentType.serie) ...[
              Text(l10n.seasonManagementTitle, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(l10n.seasonsLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
                ElevatedButton(
              ...
              const SizedBox(height: 12),
              ...seasons.map((w) => w.season).toList(),
              const SizedBox(height: 16),
            ],

            // Bande annonce
            if (_contentType != ContentType.serie) ...[
              Text(l10n.trailerLabel, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Row(children: [
                ElevatedButton(
                ...
                if (trailerFile != null && trailerPlayerController != null && trailerPlayerController!.value.isInitialized)
                  Container(
                    width: 200,
                    height: 120,
              ...
              if (trailerFile != null && trailerPlayerController != null && trailerPlayerController!.value.isInitialized) ...[
                ...
                IconButton(
                  icon: Icon(
                    trailerPlayerController!.value.isPlaying ? Icons.pause : Icons.play_arrow,
                    size: 36,
                  ),
                  onPressed: () {
                    setState(() {
            ...
            if (_contentType != ContentType.serie) ...[
              ...
              Text(l10n.mediaFilesTitle, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),

              Text(l10n.posterLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Row(children: [
                ElevatedButton(
              ...
              Text(l10n.bannerLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Row(children: [
                ElevatedButton(
              ...
              Text(l10n.mainVideoLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Row(children: [
                ElevatedButton(
                ...
                const SizedBox(width: 16),
                if (videoFile != null && videoPlayerController != null && videoPlayerController!.value.isInitialized)
                  Container(
                    width: 200, 
                    height: 120, 
                    decoration: BoxDecoration(
...
class SeasonForm extends StatefulWidget {
  ...
  const SeasonForm({Key? key, required this.seasonNumber, required this.onRemove}) : super(key: key);
...
class SeasonState extends State<SeasonForm> {
  ...
  void disposeControllers() {
    ...
    trailerPlayerController?.dispose();
    for (final w in episodes) w.key.currentState?.disposeControllers();
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Trailer
            Text(l10n.seasonTrailerLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Row(
              children: [
                ElevatedButton(
            ...
            const SizedBox(height: 12),
            Text(l10n.episodesLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),

            ...episodes.map((w) => w.episode).toList(),
...
class EpisodeForm extends StatefulWidget {
  ...
  final VoidCallback onRemove;
  const EpisodeForm({Key? key, required this.episodeNumber, required this.onRemove}) : super(key: key);
