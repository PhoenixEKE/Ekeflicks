...
# Contrats Producteurs signes - stockage definitif prive.
B2_CONTRACT_BUCKET = os.environ.get(
    "B2_CONTRACT_BUCKET",
    "ekeflicks-contracts",
)
...
STORAGES = {
    ...
    # Contrats Producteurs : aucun custom_domain/public URL.
    # Les fichiers sont servis exclusivement par une vue Django authentifiee.
    "producer_contracts": {
        "BACKEND": "storages.backends.s3.S3Storage",
        "OPTIONS": {
            **b2_storage_options(B2_CONTRACT_BUCKET),
...
PRODUCER_FRONTEND_BASE_URL = os.environ.get(
...
# =========================================================
# PRODUCER ONBOARDING / AGREEMENT
# =========================================================

PRODUCER_AGREEMENT_CURRENT_VERSION = os.environ.get(
    'PRODUCER_AGREEMENT_CURRENT_VERSION',
    '2026-09-v1',
)


# Versions antérieures dont une signature déjà réalisée continue
# d'autoriser l'accès Producteur. Cela permet une migration de
# contrat sans désactiver brutalement les producteurs existants.
PRODUCER_AGREEMENT_LEGACY_ACCEPTED_VERSIONS = tuple(
    version.strip()
    for version in os.environ.get(
        'PRODUCER_AGREEMENT_LEGACY_ACCEPTED_VERSIONS',
        '2026-09-v1',
    ).split(',')
    if version.strip()
)

PRODUCER_AGREEMENT_ACCEPTED_VERSIONS = tuple(
    dict.fromkeys(
        (
            PRODUCER_AGREEMENT_CURRENT_VERSION,
            *PRODUCER_AGREEMENT_LEGACY_ACCEPTED_VERSIONS,
...
PRODUCER_AGREEMENT_TITLE = os.environ.get(
    'PRODUCER_AGREEMENT_TITLE',
    'Contrat Producteur EKEFLICKS',
)

PRODUCER_AGREEMENT_DOCUMENT_URL = os.environ.get(
    'PRODUCER_AGREEMENT_DOCUMENT_URL',
    '',
)

PRODUCER_AGREEMENT_SHA256 = os.environ.get(
    'PRODUCER_AGREEMENT_SHA256',
    '',
).strip().lower()


EKEFLICKS_CONTRACT_LEGAL_NAME = os.environ.get(
    'EKEFLICKS_CONTRACT_LEGAL_NAME',
    'EKEFLICKS',
)

EKEFLICKS_CONTRACT_ADDRESS = os.environ.get(
    'EKEFLICKS_CONTRACT_ADDRESS',
    '',
)

EKEFLICKS_CONTRACT_REGISTRATION = os.environ.get(
    'EKEFLICKS_CONTRACT_REGISTRATION',
    '',
)

EKEFLICKS_CONTRACT_SIGNER_NAME = os.environ.get(
    'EKEFLICKS_CONTRACT_SIGNER_NAME',
    '',
)

EKEFLICKS_CONTRACT_SIGNER_ROLE = os.environ.get(
