...
class ProducerEngagementAnalyticsTests(
    ...
    def test_reuses_d6_and_d4_contract(
        ...
        # ClickHouse must not reuse profile_id as the alias
        # of any(profile_id), otherwise alias expansion can
        # turn the WHERE profile_id predicate into an illegal
        # aggregate-in-WHERE expression.
        self.assertIn(
            'any(profile_id)',
            query,
        )
