...
from core.models import Payment, PaymentWebhookEvent, ProducerPayoutRequest, Subscription, SubscriptionPlan
...
class PaymentWebhookView(views.APIView):
    ...
    def post(self, request, provider):
        ...
        signature_valid = verify_webhook_signature(provider, raw_body, headers)
        ...
        if not signature_valid:
            webhook_event.error_message = 'Signature webhook invalide ou secret non configure.'
            webhook_event.save(update_fields=['error_message', 'updated_at'])
            return Response({'detail': webhook_event.error_message}, status=status.HTTP_400_BAD_REQUEST)

        payment, error = apply_verified_payment_event(provider, normalized, payload)
        if payment:
        ...
        webhook_event.processed_at = timezone.now()
        webhook_event.save(update_fields=['error_message', 'processed_at', 'updated_at'])
        return Response({'status': 'accepted', 'detail': error}, status=status.HTTP_202_ACCEPTED)
