...
class SubscriptionPlanAdminSerializer(serializers.ModelSerializer):
    ...
    def validate_currency(self, value):
        ...
        if len(value) != 3:
            raise serializers.ValidationError(
                'La devise doit utiliser un code ISO de 3 lettres.'
            )
    ...
    def validate_duration_days(self, value):
        if value <= 0:
            raise serializers.ValidationError(
                'La durée doit être supérieure à 0 jour.'
            )

        return value


class SubscriptionPlanOfferAdminSerializer(serializers.ModelSerializer):
    plan_slug = serializers.CharField(source='plan.slug', read_only=True)
    plan_name = serializers.CharField(source='plan.name', read_only=True)

    class Meta:
        model = SubscriptionPlanOffer
        fields = [
            'id',
            'plan',
            'plan_slug',
...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        currency = subscription.currency_at_purchase

        # Compatibilite temporaire avec les abonnements historiques
        # qui ne possedent pas encore de snapshot.
        if amount is None:
            amount = plan.price
        ...
        if stripe_currency in zero_decimal_currencies:
            unit_amount = int(Decimal(amount))
        else:
        ...
        try:
            ...
            raise serializers.ValidationError(
                {
                    'provider': (
                        f'Impossible d initialiser Stripe: {str(exc)}'
                    )
                }
            )
