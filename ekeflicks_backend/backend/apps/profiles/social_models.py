...
class SocialConnection(models.Model):
    """
    Symmetric social relationship initiated by a directional request.

    requester -> target while pending.
    Once accepted, the relationship is treated as symmetric by services/API.

    PostgreSQL owns connection authorization and persistence.
    """
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name="social_connection_unique_direction",
            ),
            models.CheckConstraint(
                check=~models.Q(
                    requester=models.F("target"),
                ),
                name="social_connection_no_self",
            ),
        ]
        indexes = [
            models.Index(
                fields=("requester", "status"),
                name="social_conn_requester_idx",
            ),
            models.Index(
                fields=("target", "status"),
                name="social_conn_target_idx",
            ),
        ]
