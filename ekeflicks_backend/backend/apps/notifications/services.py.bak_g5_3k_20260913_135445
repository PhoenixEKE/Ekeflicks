...
from apps.notifications.serializers import notification_preferences
...
EVENT_DEFAULTS = {
    "account_created": (
        ...
        "Votre compte EkeFlicks a ete cree.",
    ),
    "email_verification": (
        "Validez votre email",
        "Cliquez sur le lien de validation envoye par EkeFlicks.",
    ),
    "password_reset": (
    ...
    "password_changed": (
        ...
        "Votre mot de passe EkeFlicks a bien ete modifie.",
    ),
    "parental_pin_reset": (
        "Reinitialisation du PIN parental",
        "Utilisez le lien pour modifier votre PIN parental.",
    ),
    "parental_pin_changed": (
        "PIN parental modifie",
        "Votre PIN parental EkeFlicks a bien ete modifie.",
    ),
    "email_change_support_requested": (
        "Demande de changement email recue",
        "Le support EkeFlicks traitera votre demande.",
    ),
    "email_change_support_resolved": (
        "Demande de changement email resolue",
        "Le support EkeFlicks a traite votre demande.",
    ),
    "email_change_support_rejected": (
        "Demande de changement email refusee",
        "Le support EkeFlicks a refuse votre demande.",
    ),
    "subscription_created": (
        "Abonnement cree",
        "Votre demande d abonnement a ete creee.",
    ),
    ...
    "content_published": (
        "Nouveau contenu publie",
        "Un nouveau film ou une nouvelle serie est disponible.",
    ),
    "producer_payout_requested": (
        "Demande de paiement recue",
        "Votre demande de paiement producteur a ete creee.",
    ),
    "producer_payout_approved": (
        "Paiement producteur valide",
        "Votre demande de paiement producteur a ete validee.",
    ),
    "producer_payout_rejected": (
        "Paiement producteur refuse",
        "Votre demande de paiement producteur a ete refusee.",
    ),
...
def _unsubscribe_url(user):
    ...
    return f"{base_url}/unsubscribe?token={token}" if base_url else ""


def _email_html(title, message, unsubscribe_url=""):
    ...
    logo = (
        ...
        if logo_url
        else '<img src="cid:logo_dark.png" width="150" alt="EkeFlicks" style="display:block;border:0">'
    )
    unsubscribe = (
        f'<br><a href="{escape(unsubscribe_url)}" style="color:#aaa">Gérer mes préférences ou me désabonner</a>'
        if unsubscribe_url
        else ""
    )
...
<tr><td style="padding:22px 36px;color:#888;font-size:12px;border-top:1px solid #333">EkeFlicks · Votre divertissement, partout.{unsubscribe}</td></tr>
...
def notify_user(user, event_name, title="", message="", data=None, email_enabled=True):
    ...
    category_enabled = preferences["categories"].get(
        EVENT_CATEGORIES.get(event_name, "account"), True
    )
    should_email = (
        email_enabled
        and category_enabled
        and preferences["email_enabled"]
        and notification_type.is_email_enabled
        and bool(user.email)
    )
    if should_email:
        try:
            ...
            email.attach_alternative(
                _email_html(
                    notification.title, notification.message, _unsubscribe_url(user)
                ),
                "text/html",
            )
