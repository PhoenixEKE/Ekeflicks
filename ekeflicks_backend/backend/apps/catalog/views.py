...
from apps.catalog.media_services import publish_content_media
    ...
    def approve_submission(self, request, pk=None):
        ...
        if content.producer:
            notify_user(
                content.producer,
                'content_approved',
                message=content.review_reason or f"{content.title} a ete valide.",
                data={'content_id': str(content.id)},
            )
        return Response(self.get_serializer(content).data)
