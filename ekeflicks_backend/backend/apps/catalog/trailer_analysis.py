...
def schedule_trailer_analysis(target):
    ...
    with transaction.atomic():
        ...
        # update_or_create() may update a different Python
        # instance from the TrailerAnalysisReport object already
        # cached on target.trailer_analysis_report.
        #
        # Keep the reverse OneToOne cache coherent immediately:
        # serializers called in the same request must expose the
        # newly reset PENDING report, never stale PASSED metadata.
        if isinstance(target, Content):
            relation = (
                TrailerAnalysisReport._meta
                .get_field("content")
                .remote_field
            )
        else:
