...
class CatalogApiTests(APITestCase):
    ...
    def test_staff_can_upload_content_media_assets(self):
        ...
        self.client.force_authenticate(user=user)
        cases = [
            ('content-upload-poster', 'poster.jpg', b'poster', 'poster_url', 'posters', 'poster.jpg'),
            ('content-upload-backdrop', 'backdrop.jpg', b'backdrop', 'backdrop_url', 'backdrops', 'backdrop.jpg'),
            ('content-upload-trailer', 'trailer.mp4', b'trailer', 'trailer_url', 'trailers', 'trailer.mp4'),
        ]

        for route_name, filename, payload, field, cdn_prefix, final_name in cases:
            ...
            expected_url = f'https://cdn.ekeflicks.com/{cdn_prefix}/{self.content.id}/{final_name}'
            self.assertEqual(response.data['url'], expected_url)
            self.content.refresh_from_db()
            self.assertEqual(getattr(self.content, field), expected_url)
