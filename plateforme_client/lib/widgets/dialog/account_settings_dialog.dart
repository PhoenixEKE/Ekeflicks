...
class _AccountSettingsDialogState extends State<AccountSettingsDialog> {
  ...
  Future<void> _save() async {
    ...
    if (_adultProfilesLocked && !_hasPin && !RegExp(r'^\d{4,6}$').hasMatch(pin)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Le PIN doit contenir entre 4 et 6 chiffres.')),
      );
      return;
    }
    if (_hasPin && pin.isNotEmpty && !RegExp(r'^\d{4,6}$').hasMatch(_oldPinController.text)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Saisissez votre ancien PIN pour le modifier.')),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Impossible d’enregistrer les paramètres : $error')),
        );
      }
    } finally {
  ...
  Future<void> _forgotPin() async {
    ...
    if (email.isEmpty) {
      ...
      email = await showDialog<String>(
            ...
            builder: (dialogContext) => AlertDialog(
              ...
              actions: [
                ...
                FilledButton(onPressed: () => Navigator.pop(dialogContext, controller.text.trim()), child: const Text('Continuer')),
              ],
            ),
          ) ??
          '';
      ...
      if (email.isEmpty || !RegExp(r'^[^@\s]+@[^@\s]+\.[^@\s]+$').hasMatch(email)) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Adresse e-mail invalide.')));
        return;
      }
      await userProvider.updatePersonalInfo(email: email, phone: userProvider.accountPhone);
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Un lien de modification du PIN a été envoyé à $email.')),
      );
    }
  }
            ...
            body: ListView(
              ...
              children: [
                ...
                TextField(
                  ...
                  maxLength: 6,
                  enabled: _hasPin,
                  decoration: const InputDecoration(labelText: 'Ancien PIN', prefixIcon: Icon(Icons.lock_outline)),
                ),
                TextField(
                  ...
                  keyboardType: TextInputType.number,
                  maxLength: 6,
                  decoration: const InputDecoration(labelText: 'Nouveau PIN (4 à 6 chiffres)', prefixIcon: Icon(Icons.pin)),
                ),
                if (_hasPin)
                  Align(
                    alignment: Alignment.centerRight,
                    child: TextButton(onPressed: _forgotPin, child: const Text('PIN oublié ? Recevoir un lien par e-mail')),
                  ),
                SwitchListTile(
                ...
                DropdownButtonFormField<int>(
                  ...
                  decoration: const InputDecoration(labelText: 'Classification maximale'),
                  items: [3, 7, 10, 13, 16, 18].map((age) => DropdownMenuItem(value: age, child: Text('$age ans'))).toList(),
                  onChanged: (value) => setState(() => _maximumAge = value ?? 13),
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  value: ['FR', 'CI', 'SN', 'CM', 'US'].contains(_territory) ? _territory : 'FR',
                  decoration: const InputDecoration(labelText: 'Territoire de classification'),
                  items: const {'FR': 'France', 'CI': 'Côte d’Ivoire', 'SN': 'Sénégal', 'CM': 'Cameroun', 'US': 'États-Unis'}
                ...
                SwitchListTile(value: _childHistoryEnabled, onChanged: (v) => setState(() => _childHistoryEnabled = v), title: const Text('Historique enfant')),
                SwitchListTile(value: _safeSearchEnabled, onChanged: (v) => setState(() => _safeSearchEnabled = v), title: const Text('Recherche adaptée aux enfants'), subtitle: const Text('Masque les résultats dépassant la classification autorisée.')),
                ...
                SwitchListTile(value: _notificationsEnabled, onChanged: (v) => setState(() => _notificationsEnabled = v), title: const Text('Notifications')),
              ],
            ),
  ...
  Widget _title(String label, IconData icon) => Padding(
        ...
        child: Row(children: [Icon(icon, color: AppTheme.primaryOrange), const SizedBox(width: 8), Text(label, style: Theme.of(context).textTheme.titleLarge)]),
      );
}
