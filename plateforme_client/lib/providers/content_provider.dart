...
class ContentProvider with ChangeNotifier {
  ...
  Future<void> loadInitialContent(AppLocalizations _) async {
    ...
    try {
      ...
      _errorMessage =
          'Impossible de charger les contenus. Vérifiez votre connexion.';
  ...
  Future<List<Content>> loadList(String id) =>
      _api.listContents(id, profileId: profileId);
  ...
  Future<void> toggleFavorite(Content content) async {
    ...
    _replace(
      content.id,
      (item) => item.copyWith(isFavorite: !content.isFavorite),
    );
  }
  ...
  Future<void> saveProgress(
    ...
    await _api.saveProgress(
      ...
      position:
          ...
          Duration(
            milliseconds:
                ((duration?.inMilliseconds ?? 0) * safeProgress).round(),
          ),
  ...
  void updateProgress(String contentId, double progress) => _replace(
    contentId,
    (item) => item.copyWith(progress: progress.clamp(0, 1).toDouble()),
  );
  ...
  List<Content> getSimilarContent(Content content, {int limit = 3}) =>
      _allContent
          .where(
            (item) =>
                item.id != content.id &&
                item.genres.any(content.genres.contains),
          )
          .take(limit)
          .toList();
