...
class _PostLoginPageState extends State<PostLoginPage> {
  ...
  void _handlePlayPressed(BuildContext context, Content content, {double? startAt}) {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${loc?.videoPlaybackError ?? 'Playback error'}: ${e.toString()}'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    }
  ...
  List<Content> _generateSimilarContent(Content content) {
    ...
    return allContents
        .where((c) => c.id != content.id && c.genres.any((g) => content.genres.contains(g)))
        .take(5)
        .toList();
  }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => GenrePage(
          genre: genre,
          contents: _getContentsByGenre(genre),
        ),
      ),
    );
  }
  ...
  List<Content> _generateTrendingContent() {
    return List.generate(15, (index) => Content(
      id: 'trending_$index',
  ...
  List<Content> _generateNewReleases() {
    final loc = AppLocalizations.of(context);
    return List.generate(12, (index) => Content(
      id: 'new_$index',
  ...
  List<Content> _generateRecommendedContent() {
    final loc = AppLocalizations.of(context);
    return List.generate(10, (index) => Content(
    ...
    return [
      {
        ...
        'contents': _getContentsByGenre(loc?.action ?? 'Action').take(3).toList(),
      },
      {
        'title': loc?.comedy ?? 'Comedy',
        'imageUrl': 'https://picsum.photos/300/180?random=102',
        'contents': _getContentsByGenre(loc?.comedy ?? 'Comedy').take(3).toList(),
      },
      {
      ...
      },
      {
        ...
        'contents': _getContentsByGenre(loc?.sciFi ?? 'Sci-Fi').take(3).toList(),
      },
      {
        'title': loc?.documentary ?? 'Documentary',
        'imageUrl': 'https://picsum.photos/300/180?random=105',
        'contents': _getContentsByGenre(loc?.documentary ?? 'Documentary').take(3).toList(),
      },
      {
        'title': loc?.fantasy ?? 'Fantasy',
        'imageUrl': 'https://picsum.photos/300/180?random=106',
        'contents': _getContentsByGenre(loc?.fantasy ?? 'Fantasy').take(3).toList(),
      },
      {
        'title': loc?.horror ?? 'Horror',
        'imageUrl': 'https://picsum.photos/300/180?random=107',
        'contents': _getContentsByGenre(loc?.horror ?? 'Horror').take(3).toList(),
      },
      {
        'title': loc?.romance ?? 'Romance',
        'imageUrl': 'https://picsum.photos/300/180?random=108',
        'contents': _getContentsByGenre(loc?.romance ?? 'Romance').take(3).toList(),
      },
    ];
  }
  ...
  Widget _buildContentGrid({
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final cardWidth = deviceInfo.isTV 
          ? (isLarge ? 250.0 : 200.0)
          : (isLarge ? 200.0 : 120.0);
        
        final crossAxisCount = (availableWidth / cardWidth).floor().clamp(1, deviceInfo.isTV ? 8 : 6);
        ...
        return GridView.builder(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            ...
            childAspectRatio: deviceInfo.isTV 
  ...
  Widget _buildSection({
    ...
    final maxItemsPerRow = _calculateMaxItemsPerRow(MediaQuery.of(context).size.width, contents.length);
            ...
            children: [
              ...
              if (contents.length > maxItemsPerRow)
                TextButton(
                  ...
                  child: Text(
                    showAll ? loc?.seeLess ?? 'See less' : loc?.seeAll ?? 'See all',
                    style: TextStyle(
              ...
              slivers: [
                SliverToBoxAdapter(
                  child: HeroBanner(
                    ...
                    onPlayPressed: (content) => _handlePlayPressed(context, content),
                    onInfoPressed: (content) => _handleInfoPressed(context, content),
                  ),
                ),
                SliverPadding(
                  ...
                  sliver: SliverToBoxAdapter(
                    child: _buildSection(
                      ...
                      onContentPressed: (content) => _handlePlayPressed(
                        context, 
                        content, 
                        startAt: content.progress != null 
                          ? content.duration.inSeconds * content.progress!
                          : null,
                      ),
                    ),
                  ),
                ...
                SliverPadding(
                  ...
                  sliver: SliverToBoxAdapter(
                    child: _buildSection(
                      ...
                      section: 'trending',
                      onContentPressed: (content) => _handleInfoPressed(context, content),
                      isLarge: true,
                    ),
                  ),
                ),
                            ...
                            children: [
                              ...
                              if (popularGenres.length > 5)
                                TextButton(
                                  ...
                                  child: Text(
                                    _showAllGenres ? loc?.seeLess ?? 'See less' : loc?.seeAll ?? 'See all',
                                    style: TextStyle(
                                      color: Theme.of(context).colorScheme.primary,
                                      fontSize: deviceInfo.isTV ? 18 : 16,
                                    ),
