...
class _TechnicalMediaUploadException implements Exception {
  const _TechnicalMediaUploadException({
  ...
  String get message {
    ...
    final detailPart = cleanDetail.isEmpty ? '' : ' $cleanDetail';

    return '$mediaLabel non conforme aux critères techniques EKEFLICKS.'
        '$detailPart '
        'Consultez les critères techniques « $mediaLabel » '
        'puis sélectionnez un nouveau fichier.';
  }
...
class _UploadPageState extends State<UploadPage> {
  ...
  XFile? bannerFile;
  XFile? videoFile;
  NativeVideoFile? nativeVideoFile;
  ...
  final GlobalKey<_TechnicalRequirementsAccordionState> _posterRequirementsKey =
      GlobalKey<_TechnicalRequirementsAccordionState>();

  final GlobalKey<_TechnicalRequirementsAccordionState> _bannerRequirementsKey =
      GlobalKey<_TechnicalRequirementsAccordionState>();
  ...
  void _applyXmlMetadataToForm() {
    ...
    // L'application XML reste neutre pendant l'injection des
    // valeurs, puis déclenche une seule sauvegarde du brouillon.
    _scheduleAutoSave(immediate: true);
  ...
  Future<void> _addProducer() async {
    ...
    final person = await showDialog<PersonWithImage>(
      context: context,
      builder: (dialogContext) => ProducerModalShell(
        title: AppLocalizations.of(context)!.addProducerTitle,
        actions: [
          TextButton(
  ...
  Future<void> _addActor() async {
    ...
    final person = await showDialog<PersonWithImage>(
      context: context,
      builder: (dialogContext) => ProducerModalShell(
        title: AppLocalizations.of(context)!.addActorTitle,
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogContext),
            child: Text(AppLocalizations.of(context)!.cancelButton),
  ...
  Future<void> _pickGlobalVideo() async {
    final picked = await pickNativeVideoFile();
    ...
    final previousNativeVideo = nativeVideoFile;

    setState(() {
      nativeVideoFile = picked;
      videoFile = picked.previewFile;
    });

    previousNativeVideo?.dispose();
  ...
  Future<void> _runAutoSave() async {
    ...
    } on _TechnicalMediaUploadException catch (error) {
      _autoSaveDirty = false;

      if (mounted) {
        setState(() {
          _autoSaveStatus = 'Média non conforme — correction requise';
        });
  ...
  Future<void> _persistSeriesEpisodeVideos(String contentId) async {
    ...
    for (final seasonWrapper in seasons) {
      ...
      for (final episodeWrapper in seasonState.episodes) {
        ...
        final source = episodeState.nativeVideoFile;
        ...
        await _uploadVideoSourceMultipart(
          service: service,
          assetId: assetId,
          source: source,
          sourceLength: length,
  ...
  Future<void> _uploadVideoSourceMultipart({
    required ProducerService service,
    required String assetId,
    required NativeVideoFile source,
    ...
    final session = await service.createSourceMultipartUpload(
      assetId: assetId,
      filename: source.name,
      sizeBytes: sourceLength,
    );

    final uploadToken = session['upload_token']?.toString() ?? '';

    final partSizeValue = session['part_size'];
    final partCountValue = session['part_count'];

    if (uploadToken.isEmpty ||
        partSizeValue is! int ||
        partSizeValue <= 0 ||
        partCountValue is! int ||
        partCountValue <= 0) {
      throw StateError(
        'La session multipart de la vidéo '
        'est incomplète.',
      );
    }

    final partSize = partSizeValue;
    final partCount = partCountValue;

    final expectedPartCount = (sourceLength + partSize - 1) ~/ partSize;

    if (partCount != expectedPartCount) {
      throw StateError(
        'Le découpage multipart retourné '
        'par le serveur est incohérent.',
      );
    }

    MultipartBlobUploadSession? blobSession;

    var completed = false;

    try {
      blobSession = await MultipartBlobUploadSession.open(
        source: source.file,
        expectedSize: sourceLength,
      );

      final uploadedParts = <MultipartUploadedPart>[];
      ...
      Future<void> worker() async {
        while (true) {
          ...
          if (nextPartNumber > partCount) {
            return;
          }

          final partNumber = nextPartNumber;
          nextPartNumber += 1;

          try {
            final startByte = (partNumber - 1) * partSize;

            final calculatedEnd = startByte + partSize;

            final endByte = calculatedEnd < sourceLength
                ? calculatedEnd
                : sourceLength;

            final partSession = await service.createSourceMultipartPart(
              assetId: assetId,
            ...
            final uploadUrl =
                partSession['upload_url']?.toString().trim() ?? '';

            if (uploadUrl.isEmpty) {
              throw StateError(
                'URL d’envoi absente pour '
                'la partie $partNumber.',
              );
            }

            final uploaded = await blobSession!.uploadPart(
              uploadUrl: uploadUrl,
              partNumber: partNumber,
              startByte: startByte,
              endByte: endByte,
      ...
      if (firstFailure != null) {
        Error.throwWithStackTrace(
          firstFailure!,
          firstFailureStackTrace ?? StackTrace.current,
        );
      }

      if (uploadedParts.length != partCount) {
        throw StateError(
          'Toutes les parties de la vidéo '
      ...
      for (var index = 0; index < uploadedParts.length; index += 1) {
        final expectedPartNumber = index + 1;

        if (uploadedParts[index].partNumber != expectedPartNumber) {
          throw StateError(
            'La liste des parties envoyées '
            'est incomplète.',
          );
        }
      }

      await service.completeSourceMultipartUpload(
        assetId: assetId,
        uploadToken: uploadToken,
        parts: uploadedParts.map((part) => part.toJson()).toList(),
      );
      ...
      if (!completed) {
        try {
          await service.abortSourceMultipartUpload(
            assetId: assetId,
            uploadToken: uploadToken,
          );
        } catch (_) {
          // Best effort : l'erreur d'origine reste prioritaire.
        }
      }
    }
  }
  ...
  Future<void> _persistDraftMedia(String contentId) async {
    ...
    try {
      ...
      // Un nouveau fichier sélectionné localement doit toujours
      // remplacer le média TEMP existant et repasser par la
      ...
      if (poster != null) {
        ...
        try {
          ...
          if (mounted) {
            setState(() => posterFile = null);
          } else {
            posterFile = null;
          }

          await _scrollToImageRequirements('poster');

          throw _TechnicalMediaUploadException(
            mediaLabel: 'Affiche',
      ...
      // Même contrat que l'affiche : toute nouvelle sélection
      // locale doit être réellement envoyée et contrôlée, même
      // lorsqu'une bannière TEMP existe déjà sur le brouillon.
      if (banner != null) {
        ...
        try {
          ...
          if (mounted) {
            setState(() => bannerFile = null);
          } else {
            bannerFile = null;
          }

          await _scrollToImageRequirements('banner');
      ...
      // G4-5B4:
      // La présence d'un ancien trailer TEMP côté serveur ne doit
      // jamais empêcher le remplacement lorsqu'un nouveau fichier
      // a explicitement été sélectionné localement.
      ...
      final master = _contentType == ContentType.film ? nativeVideoFile : null;

      // G4-5B4:
      // Si un nouveau master local est sélectionné, il remplace
      // toujours la source de l'asset existant. hasMaster décrit
      // uniquement l'état serveur initial et ne doit pas bloquer
      // explicitement un remplacement demandé par le producteur.
      if (master != null) {
        ...
        final masterLength = master.size;
  ...
  _validateTechnicalConformityForSubmission() {
    if (_contentType == ContentType.film) {
      ...
      if (report.submissionBlocking) {
        return _TechnicalSubmissionValidationResult(
          message:
              'Le master du film ne peut pas être soumis : '
              '${report.displayLabel}. '
              'Une correction technique est requise avant '
              'la soumission.',
          route: _scrollToFilmMasterRequirements,
        );
      }
          ...
          if (report.submissionBlocking) {
            return _TechnicalSubmissionValidationResult(
              message:
                  ...
                  '${report.displayLabel}. '
                  'Une correction technique est requise '
                  'avant la soumission.',
              route: episodeState.scrollToMasterRequirements,
            );
          }
  ...
  Future<void> _scrollToImageRequirements(String sectionKey) async {
    GlobalKey<_TechnicalRequirementsAccordionState>? targetKey;

    if (sectionKey == 'poster') {
      targetKey = _posterRequirementsKey;
    } else if (sectionKey == 'banner') {
      targetKey = _bannerRequirementsKey;
    }
  ...
  Future<void> _confirmRemovePerson<T>(List<T> list, T item) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => ProducerModalShell(
        title: AppLocalizations.of(context)!.confirmationTitle,
        maxWidth: 520,
        actions: [
          TextButton(
        ...
        child: Text(AppLocalizations.of(context)!.confirmationMessage),
      ),
    );
    if (ok == true) {
        ...
        child: Column(
          ...
          children: [
                          ...
                          final contentId = await showDialog<String>(
                            ...
                            builder: (dialogContext) {
                              return const ProducerModalShell(
                                ...
                                child: ContentsPage(selectionMode: true),
                              );
                            },
                          );
            ...
            ValueListenableBuilder<Map<String, dynamic>?>(
              valueListenable: _technicalSpecificationNotifier,
              builder: (context, specification, _) {
                final version = TechnicalSpecificationContextEngine.version(
                  specification,
                );
                ...
                final sectionTitle =
                    TechnicalSpecificationContextEngine.sectionTitle(
                      specification,
                      'xml',
                    );
                ...
                return Container(
                  ...
                  child: _TechnicalRequirementsAccordion(
                    title: version.isEmpty ? xmlTitle : '$xmlTitle — V$version',
                    children: [
                      Card(
                        margin: const EdgeInsets.only(bottom: 16),
                        child: Padding(
                      ...
                      _TechnicalXmlRequirements(
                        specificationListenable:
                            _technicalSpecificationNotifier,
                      ),
                    ],
                  ),
                );
            ...
            if (_contentType != ContentType.serie) ...[
              ...
              _TechnicalImageRequirements(
                specificationListenable: _technicalSpecificationNotifier,
                sectionKey: 'poster',
                accordionKey: _posterRequirementsKey,
              ),
              ...
              _TechnicalImageRequirements(
                specificationListenable: _technicalSpecificationNotifier,
                sectionKey: 'banner',
                accordionKey: _bannerRequirementsKey,
              ),
...
class EpisodeState extends State<EpisodeForm> {
  ...
  Future<NativeVideoFile?> _pickVideo() {
    return pickNativeVideoFile();
  }
  ...
  Future<void> _showReport(BuildContext context) {
    return showDialog<void>(
      ...
      builder: (dialogContext) {
        ...
        return ProducerModalShell(
          title: 'Rapport de conformité technique',
          actions: [
            TextButton(
              onPressed: () => Navigator.of(dialogContext).pop(),
              child: const Text('FERMER'),
