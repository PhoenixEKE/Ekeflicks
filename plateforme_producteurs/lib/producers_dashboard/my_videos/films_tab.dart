...
class FilmsTab extends StatefulWidget {
  const FilmsTab({super.key, this.onEditContent});
...
class _FilmsTabState extends State<FilmsTab> {
  ...
  Future<void> _deleteDraft(Map<String, dynamic> film) async {
    if (_deletingDraftId != null || _status(film) != 'draft') {
      return;
    }

    final id = film['id']?.toString().trim() ?? '';
    if (id.isEmpty) {
      return;
    }
    ...
    try {
      ...
      setState(() {
        _films = _films.where((item) => item['id']?.toString() != id).toList();
  ...
  String _updatedAt(Map<String, dynamic> film) {
    final raw = film['updated_at']?.toString();
  ...
  Widget _buildFilmCard(Map<String, dynamic> film) {
    final status = _status(film);
    final updatedAt = _updatedAt(film);
    final deletionWarning = status == 'draft'
        ? _draftDeletionWarning(film)
        : null;
    final editable = status == 'draft' || status == 'rejected';

    final reviewReason = film['review_reason']?.toString().trim() ?? '';
                            ...
                            if (status == 'draft')
                              OutlinedButton.icon(
                                onPressed: _deletingDraftId != null
                                    ? null
                                    : () => _deleteDraft(film),
                                icon: _deletingDraftId == film['id']?.toString()
                                    ? const SizedBox(
                                        width: 18,
                                        height: 18,
