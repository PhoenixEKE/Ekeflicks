...
class _TechnicalRequirementLine extends StatefulWidget {
  ...
  @override
  State<_TechnicalRequirementLine> createState() =>
      _TechnicalRequirementLineState();
}

class _TechnicalRequirementLineState extends State<_TechnicalRequirementLine> {
  bool _isExpanded = false;

  @override
  Widget build(BuildContext context) {
    final prefix = widget.blocking
        ? 'Obligatoire'
        : widget.warning
        ? 'Recommandé'
        : null;
...
class _UploadPageState extends State<UploadPage> {
  ...
  final GlobalKey _filmMasterRequirementsKey = GlobalKey();
  ...
  String? _validateTechnicalConformityForSubmission() {
    if (_contentType == ContentType.film) {
      if (!_serverHasMaster) {
        return 'Le master vidéo du film est obligatoire avant la soumission.';
      }

      final report = _movieTechnicalConformity;

      if (report == null) {
        return 'L’analyse technique du master du film doit être terminée '
            'avant la soumission.';
      }

      if (report.displayStatus != TechnicalConformityDisplayStatus.conform) {
        return 'Le master du film ne peut pas être soumis : '
            '${report.displayLabel}. '
    ...
    if (_contentType == ContentType.serie) {
      for (final seasonWrapper in seasons) {
        ...
        if (seasonState == null) {
          return 'Impossible de vérifier la conformité technique '
              'd’une saison.';
        }
        ...
        for (final episodeWrapper in seasonState.episodes) {
          ...
          if (episodeState == null) {
            return 'Impossible de vérifier le master '
                'S$seasonNumber E$episodeNumber.';
          }

          if (!episodeState.serverHasVideo) {
            return 'Le master vidéo S$seasonNumber '
          ...
          if (report == null) {
            return 'L’analyse technique du master '
          ...
          if (report.displayStatus !=
            ...
            return 'Le master S$seasonNumber '
  ...
  Future<void> _scrollToFilmMasterRequirements() {
  ...
  Future<void> _saveForm() async {
    ...
    try {
      ...
      if (technicalConformityError != null) {
        throw StateError(technicalConformityError);
      }
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    KeyedSubtree(
                      key: _filmMasterRequirementsKey,
                      child: _TechnicalMasterRequirements(
...
class EpisodeState extends State<EpisodeForm> {
  final GlobalKey _masterRequirementsKey = GlobalKey();
  ...
  Future<void> scrollToMasterRequirements() {
